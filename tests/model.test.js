...
test("profiles select a real engine adapter while sharing one normalized UI contract", () => {
  ...
  assert.match(controller, /function tunnelLaunchCommand\(\)/)
  assert.match(controller, /pkill -f '\[E\]xitOnForwardFailure=yes/)
  assert.match(controller, /tunnelProc\.command = tunnelLaunchCommand\(\)/)
  assert.match(controller, /Component\.onDestruction/)
